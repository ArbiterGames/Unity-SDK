...
- (void)navigateToActiveView
{
    ...
    if ( self.withdrawComplete ) {
        ...
        [[ArbiterTracking arbiterInstance] track:@"Selected Withdraw Amount" properties:@{@"amount": [NSString stringWithFormat:@"%f", self.withdrawAmount]}];
        ...
        if (error) {
            ...
            [self.arbiter httpPost:APIWithdrawURL params:params isBlocking:YES handler:[^(NSDictionary *responseDict) {
                if ([[responseDict objectForKey:@"errors"] count]) {
                    ...
                    [[ArbiterTracking arbiterInstance] track:@"Received Withdraw Error" properties:@{@"error": message}];
                    [self handleError:message];
                    [self.nextButton removeFromSuperview];
                    self.activeViewIndex--;
                    [self navigateToActiveView];
                } else {
                    [[ArbiterTracking arbiterInstance] track:@"Received Withdraw Success"];
