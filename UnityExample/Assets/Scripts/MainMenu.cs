...
public class MainMenu : MonoBehaviour {
	...
	private static int boxHeight = buttonHeight * 7 + padding * 5;
	...
	void OnGUI() {
		...
		if(GUI.Button(new Rect(padding * 2.5f + buttonWidth / 2, buttonHeight + boxY, buttonWidth / 2 - padding / 2, buttonHeight), "IDs", buttonStyle)) {
			Application.LoadLevel ("IDs");
		}

		if(GUI.Button(new Rect(padding * 2, buttonHeight * 2 + padding + boxY, buttonWidth, buttonHeight), "Free Credits!", buttonStyle)) {
			Arbiter.SendPromoCredits( "100", OnSendPromoCreditsSuccess, ErrorHandler );
			Arbiter.DisplayWalletDashboard( OnWalletDashboardClose );
		}
		
		if(GUI.Button(new Rect(padding * 2, buttonHeight * 3 + padding * 2 + boxY, buttonWidth, buttonHeight), "Score Challenge", buttonStyle)) {
			Application.LoadLevel ("ScoreChallenge");
		}
		
		GUI.Label(new Rect(padding * 2, buttonHeight * 4 + padding * 2 + boxY, buttonWidth, buttonHeight), "Entry fee: 50 credits", labelStyle);
		if(GUI.Button(new Rect(padding * 2, buttonHeight * 4 + padding  * 3 + boxY, buttonWidth, buttonHeight), "Join Cash Tournament", buttonStyle)) {
			string betSize = "50";
		...
		if(GUI.Button(new Rect(padding * 2, buttonHeight * 5 + padding * 4 + boxY, buttonWidth / 2 - padding / 2, buttonHeight), "Updates", buttonStyle)) {
			Arbiter.ShowPreviousTournaments( OnViewPreviousTournamentsClosed );
		}
		
		if(GUI.Button(new Rect(padding * 2.5f + buttonWidth / 2, buttonHeight * 5 + padding * 4 + boxY, buttonWidth / 2 - padding / 2, buttonHeight), "Previous", buttonStyle)) {
			Arbiter.ShowPreviousTournaments( OnViewPreviousTournamentsClosed );
		}
		
		if(GUI.Button(new Rect(padding * 2, buttonHeight * 6 + padding * 5 + boxY, buttonWidth, buttonHeight), "Logout", buttonStyle)) {
